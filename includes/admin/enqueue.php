...
function enqueue_shelf_runner() {
    ...
    if ( ! is_wp_error( $response ) && 200 === wp_remote_retrieve_response_code( $response )) {
        ...
        if ( isset( $manifest['src/js/shortcode.js']['css'][0] ) ) {
            wp_enqueue_style( 'shelf-runner-style', SHELF_RUNNER_PLUGIN_DIST_URI . $manifest['src/js/shortcode.js']['css'][0], array(), SHELF_RUNNER_VERSION, 'all' );
        }
        if ( isset( $manifest['src/js/shortcode.js']['file'] ) ) {
            wp_enqueue_script( 'shelf-runner-script', SHELF_RUNNER_PLUGIN_DIST_URI . $manifest['src/js/shortcode.js']['file'], array(), SHELF_RUNNER_VERSION, true );
        }
    }
}